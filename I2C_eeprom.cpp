...
//  PUBLIC FUNCTIONS
//
I2C_eeprom::I2C_eeprom(const uint8_t deviceAddress, TwoWire *wire) : I2C_eeprom(deviceAddress, I2C_DEVICESIZE_24LC256, wire)
{
}
...
//  returns I2C status, 0 = OK
int I2C_eeprom::setBlock(const uint32_t memoryAddress, const uint8_t data, const uint16_t length)
{
...
//  returns I2C status, 0 = OK
int I2C_eeprom::writeBlock(const uint32_t memoryAddress, const uint8_t *buffer, const uint16_t length)
{
...
//  returns 0 == OK
int I2C_eeprom::updateByte(const uint32_t memoryAddress, const uint8_t data)
{
...
uint16_t I2C_eeprom::updateBlock(const uint32_t memoryAddress, const uint8_t *buffer, const uint16_t length)
{
  uint16_t address = memoryAddress;
  uint16_t len = length;
  uint16_t bytes = 0;
...
bool I2C_eeprom::writeByteVerify(const uint32_t memoryAddress, const uint8_t value)
{
  if (writeByte(memoryAddress, value) != 0)
    return false;
...
//  return false if write or verify failed.
bool I2C_eeprom::writeBlockVerify(const uint32_t memoryAddress, const uint8_t *buffer, const uint16_t length)
{
  if (writeBlock(memoryAddress, buffer, length) != 0)
    return false;
  return verifyBlock(memoryAddress, buffer, length);
}

//  return false if write or verify failed.
bool I2C_eeprom::setBlockVerify(const uint32_t memoryAddress, const uint8_t value, const uint16_t length)
{
  if (setBlock(memoryAddress, value, length) != 0)
    return false;
...
//  return false if write or verify failed.
bool I2C_eeprom::updateByteVerify(const uint32_t memoryAddress, const uint8_t value)
{
  if (updateByte(memoryAddress, value) != 0)
    return false;
  uint8_t data = readByte(memoryAddress);
  return (data == value);
}
...
//  return false if write or verify failed.
bool I2C_eeprom::updateBlockVerify(const uint32_t memoryAddress, const uint8_t *buffer, const uint16_t length)
{
  //  zero bytes written means nothing needs to be verified
  if (0 == updateBlock(memoryAddress, buffer, length))
    return true;
  return verifyBlock(memoryAddress, buffer, length);
}
...
uint32_t I2C_eeprom::determineSize(const bool debug)
{
  ...
  uint8_t patAA = 0xAA;
  uint8_t pat55 = 0x55;
  uint32_t maxSize = 131072; // 128kB

  for (uint32_t size = 128; size <= maxSize; size *= 2)
  {
    bool folded = false;
  ...
  // If no folding detected, assume device is at least as big as the largest tested size
  return maxSize;
}
...
uint16_t I2C_eeprom::getPageSize()
{
  return _pageSize;
}

uint16_t I2C_eeprom::getPageSize(uint32_t deviceSize)
{
  //  determine page size from device size
  //  based on Microchip 24LCXX data sheets.
  if (deviceSize <= I2C_DEVICESIZE_24LC02)
    return 8;
  if (deviceSize <= I2C_DEVICESIZE_24LC16)
    return 16;
  if (deviceSize <= I2C_DEVICESIZE_24LC64)
    return 32;
  if (deviceSize <= I2C_DEVICESIZE_24LC256)
    return 64;
  if (deviceSize <= I2C_DEVICESIZE_24LC512)
    return 128;
...
uint32_t I2C_eeprom::setDeviceSize(uint32_t deviceSize)
{
  uint32_t size = 128;
  //  force power of 2.
  while ((size <= 131072) && (size <= deviceSize))
  {
    _deviceSize = size;
    size *= 2;
  }
...
uint8_t I2C_eeprom::setPageSize(uint16_t pageSize)
{
  // force power of 2.
  if (pageSize >= 256)
  {
...
void I2C_eeprom::_beginTransmission(const uint32_t memoryAddress)
{
  ...
  {
    // CAT24M01 and similar: 17-bit addressing (a16 in I2C address)
    if (_deviceSize > 65536) // 128kB or larger
    {
    ...
    // Address High Byte (always bits 15:8)
...
//  returns 0 = OK otherwise error
int I2C_eeprom::_WriteBlock(const uint32_t memoryAddress, const uint8_t *buffer, const uint16_t length)
{
  _waitEEReady();
...
// pre: buffer is large enough to hold length bytes
// returns bytes read
uint8_t I2C_eeprom::_ReadBlock(const uint32_t memoryAddress, uint8_t *buffer, const uint16_t length)
{
  _waitEEReady();

  this->_beginTransmission(memoryAddress);
  int rv = _wire->endTransmission(false); // Use repeated START, not STOP!
  if (rv != 0)
  {
    return 0; // error
  }
  ...
  if (this->_isAddressSizeTwoWords)
  {
    uint8_t deviceAddr = _deviceAddress;
    if (_deviceSize > 65536)
    {
      // CAT24M01 addressing: A16 bit goes into bit 1 of the I2C address
      deviceAddr |= ((memoryAddress & 0x10000) >> 15);
    }
    readBytes = _wire->requestFrom((int)deviceAddr, (int)length);
  }
  else
  {
    uint8_t address = _deviceAddress | ((memoryAddress >> 8) & 0x07);
  ...
  uint8_t count = 0;
  while (count < readBytes && _wire->available())
  {
...
bool I2C_eeprom::_verifyBlock(const uint32_t memoryAddress, const uint8_t *buffer, const uint16_t length)
{
  _waitEEReady();
